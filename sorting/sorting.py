...
def plot_arr(arr):
    plt.axis('off')
    plt.bar(range(len(arr)), arr, width=1., edgecolor='black')
    plt.show()
...
def bucket_sort(arr, buckets=10):
    ...
    for i, x in enumerate(arr, start=1):
        ...
        plot_arr([x for sub in slots for x in sub] + arr[i:])
    ...
    for slot in slots:
        for j, x in enumerate(slot, start=-1):
            while j >= 0 and slot[j] > x: 
                ...
                j -= 1
            slot[j + 1] = x
            plot_arr([x for sub in slots for x in sub])
        for y in slot:
            arr[i] = y
            i += 1
